...
f.rev(psc, 0.12, 0.8) %>%
  bind_rows(f.rev(state_superx, 0.12, 0.8)) %>%  
  bind_rows(f.rev(sq, 0.12, 0.8)) %>%  
  group_by(sim, d) %>%
  summarise(rev = sum(n_rev/1000000)) %>%
  group_by(d) %>%
...
f.rev(psc, 0.12, 0.8) %>%
  bind_rows(f.rev(state_llp, 0.12, 0.8)) %>%  
  bind_rows(f.rev(sq, 0.12, 0.8)) %>%  
  group_by(sim, d) %>%
  summarise(rev = sum(n_rev/1000000)) %>%
  group_by(d) %>%
...
f.rev(psc, 0.12, 0.8) %>%
  bind_rows(f.rev(state_ecs, 0.12, 0.8)) %>%  
  bind_rows(f.rev(sq, 0.12, 0.8)) %>%  
  group_by(sim, d) %>%
  summarise(rev = sum(g_rev/1000000)) %>%
  group_by(d) %>%
  mutate(mean = mean(rev)) %>%
  ggplot(aes(sim, rev, color=factor(d))) + geom_point() +
  geom_vline(xintercept=20, lty=4, alpha = 0.5) +
...
f.rev(cq_fed, 0.12, 0.8) %>%
  bind_rows(f.rev(state_llp, 0.10, 0.7)) %>%  
