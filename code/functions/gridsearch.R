...
f.gridfuel <- function(x, penalty){
  # assumed number of days to catch 50 t fish
  assume_day = 2.5
  # for each vessel size determine the anticipated fuel consumption
  fuel = ifelse(x[1]==9, 0,
...
f.gridexv <- function(x){
  ...
  # for each port set an anticipated exvessel price
  if(x[4]==1) {ex1
  } else if(x[4]==2) {ex2
  } else if(x[4]==3) {ex3
  } else {ex4}
...
f.gridrev <- function(gridfuel, gridexv){
  # calculate revenue based upon assumed travel time and exvessel price
  assumed_catch = 50
...
f.grid <- function(x){
  # roll all of the functions together so they may be optimized
  penalty = f.penalty(x)
